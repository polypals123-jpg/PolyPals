...
/*********************
 * AUTH & SESSION HANDLER — unified for all roles
 *********************/
firebase.auth().onAuthStateChanged(async (user) => {
  if (!user) return;

  // 🧠 Read new session keys (primary)
  let uid = localStorage.getItem("userUID");
  let polyId = localStorage.getItem("userHomePolyId");
  let role = localStorage.getItem("userRole");

  // 🧩 Backward compatibility (old keys fallback)
  if (!uid) uid = localStorage.getItem("uid");
  if (!polyId) polyId = localStorage.getItem("polyId");
  if (!role) role = localStorage.getItem("role");

  // 🩵 If still missing poly/role, auto-detect from Firestore
  if (!polyId || !role) {
    try {
      ...
      for (const polyDoc of polysSnap.docs) {
        ...
        const staffRef = polyDoc.ref.collection("staffs").doc(user.uid);
      ...
      if (polyId && role) {
        // ✅ Save in both new + old keys
        localStorage.setItem("userUID", uid || user.uid);
        localStorage.setItem("userHomePolyId", polyId);
        localStorage.setItem("userRole", role);
        localStorage.setItem("polyId", polyId); // legacy for old scripts
        localStorage.setItem("role", role);
        console.log(`✅ Session restored: ${polyId} | ${role}`);
      } else {
    ...
    // ✅ Always ensure both formats exist
    localStorage.setItem("polyId", polyId);
    localStorage.setItem("role", role);
  }

  console.log("📦 Session check (home.js):", { uid, polyId, role });
});
