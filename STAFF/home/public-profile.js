...
async function findUserDocRef(uid) {
  for (const poly of allPolys) {
    ...
    let staffRef = polyRef.collection("staffs").doc(uid);
...
async function openUserListPopup(uid, type) {
  ...
  content.innerHTML = "<p style='opacity:0.7;'>Loading...</p>";
    ...
    let snap = await db.collection("polys").doc(polyId)
      .collection("students").doc(uid)
      .collection(type).get();

    if (snap.empty) {
      snap = await db.collection("polys").doc(polyId)
        .collection("staffs").doc(uid)
        .collection(type).get();
  }
